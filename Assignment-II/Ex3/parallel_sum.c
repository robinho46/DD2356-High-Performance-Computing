...
double omp_local_sum(double *input, size_t size, int num_threads){
...
int main() {
    ...
    for (int thread_index = 0; thread_index < num_threads; thread_index++) {
        ...
        double times[10]; // Array to store times for each test
        for (int t = 0; t < 10; t++) {
            double start_time = omp_get_wtime();
            double sum = omp_critical_sum(input, SIZE);  // Change this call for each function version
